...
export function allTargetFixtures(
  targetKeys: readonly string[],
  now: number = Date.now(),
): Record<string, FixtureRepoSnapshot> {
  const out: Record<string, FixtureRepoSnapshot> = {};
  for (const key of targetKeys) out[key] = buildFixtureForTarget(key, now);
  return out;
}
